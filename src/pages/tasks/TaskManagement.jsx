...
import { myTask, deligatedTask, allTask, viewTask, filterTask } from "../../api/tasksApi"
...
const TaskManagement = () => {
  ...
  const applyFiltersWithAPI = async () => {
    try {
      ...
      const filterParams = {}
      if (filters.taskCategory !== "all") filterParams.taskCategory = filters.taskCategory
      if (filters.taskPriority !== "all") filterParams.taskPriority = filters.taskPriority
      if (filters.taskStatus !== "all") filterParams.taskStatus = filters.taskStatus
      ...
      try {
        ...
        taskData.forEach((task) => {
          ...
          if (id) {
            ...
            console.warn("[fetchAllTasksData] Task from allTask API without a valid ID, skipping:", task)
          }
        })
      ...
      {showFilters && (
        <div className="mb-6 p-4 bg-gray-50 rounded-lg border">
          <div className="flex flex-wrap items-center gap-4 mb-4">
            ...
            {hasActiveFilters() && (
              <Button onClick={clearAllFilters} variant="outline" size="sm" className="flex items-center gap-2 mt-6">
                <X className="h-4 w-4" />
                Clear Filters
              </Button>
            )}
