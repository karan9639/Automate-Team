...
    // Fallback to whatever ID we can find in the task
    // Direct _id field (string format)
    if (task._id && typeof task._id === "string") {
      return task._id
    }
...
  // First, fetch All Tasks to build the ID mapping
...
      // Now fetch the tasks for the active tab
...
      // NORMALIZE: Make task._id the correct string ID from All Tasks
...
      // Always fetch All Tasks when switching to the tab (not just when empty)
      const response = await allTask()

      const taskData = response?.data?.data?.tasks || extractTasksFromResponse(response)

      // Normalize all tasks (should already have correct IDs)
      const normalizedAllTasks = taskData.map((task) => {
        const id = extractAllTaskId(task)
        if (id) {
          return { ...task, _id: id }
