...
import { TrendingUp, Calendar, Filter, RefreshCw, Clock } from "lucide-react"
    ...
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Welcome back! Here's what's happening with your Automate Tasks.</p>
          ...
          {lastOverdueCheck && (
            <p className="text-xs text-gray-500 mt-1">
              Last overdue check: {new Date(lastOverdueCheck).toLocaleTimeString()}
            </p>
          )}
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            <Clock className={`h-4 w-4 ${isCheckingOverdue ? "animate-spin" : ""}`} />
          ...
          >
            <RefreshCw className={`h-4 w-4 ${isRefreshing || activitiesLoading ? "animate-spin" : ""}`} />
            Refresh
          </Button>
          <Button size="sm" className="flex items-center gap-2" onClick={() => setIsGenerateReportOpen(true)}>
      ...
      {hasActiveFilters() && (
        <div className="flex items-center gap-2 flex-wrap p-4 bg-blue-50 rounded-lg border border-blue-200">
          ...
          {searchQuery && (
            <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">Search: "{searchQuery}"</span>
          )}
          {statusFilter !== "All" && (
            <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">Status: {statusFilter}</span>
          )}
          {userFilter !== "All" && (
            <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">Assigned to: {userFilter}</span>
          )}
          {priorityFilter !== "All" && (
            <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">Priority: {priorityFilter}</span>
          )}
          {categoryFilter !== "All" && (
            <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">Category: {categoryFilter}</span>
          )}
          {frequencyFilter !== "All" && (
          ...
          <Button variant="ghost" size="sm" className="text-blue-600 hover:text-blue-800" onClick={clearAllFilters}>
            Clear All
          </Button>
        </div>
      )}
