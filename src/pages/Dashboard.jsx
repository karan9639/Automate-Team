...
const Dashboard = () => {
  ...
  const executeOverdueCheck = async (showToast = true) => {
    ...
    try {
      ...
      if (response.data && response.data.updatedTasks) {
        ...
        if (showToast) {
          toast.success(
            `Overdue check completed. ${updatedTaskIds.length} task(s) updated.`
          );
        }
        ...
        const newActivity = {
          ...
          task:
            updatedTaskIds.length === 1
              ? "task"
              : `${updatedTaskIds.length} tasks`,
  ...
  const calculateKPIs = (taskList) => {
    ...
    return {
      ...
      completedTasks: taskList.filter(
        (task) => completedStatuses.includes(task.status) || task.completedAt
      ).length,
  ...
  const handleTaskStatusChange = async (taskId, newStatus) => {
    try {
      ...
      if (wasOverdue) {
        if (isCompleting) {
          toast.success(
            `Task "${updatedTask.name}" completed and removed from overdue list`
          );
        } else {
          toast.success(
            `Task "${updatedTask.name}" status changed to ${newStatus} and removed from overdue list`
          );
        }
      }
  ...
  return (
    <div className="space-y-6 md:space-y-8 p-4 md:p-8 relative">
        ...
        <div>
          ...
          {lastOverdueCheck && (
            <p className="text-xs text-muted-foreground/80 mt-1">
              Overdue tasks last checked:{" "}
              {new Date(lastOverdueCheck).toLocaleTimeString()}
            </p>
          )}
        </div>
      ...
      {hasActiveFilters() && (
        <div className="flex items-center gap-2 flex-wrap p-3.5 bg-accent rounded-lg border border-primary/20">
          ...
          {/* Active filter badges would go here, styled with modern Badge component */}
          <Button
            variant="ghost"
            size="sm"
