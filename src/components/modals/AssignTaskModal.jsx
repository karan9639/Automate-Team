...
import { Check, FileText, AlertCircle, Loader2, Search, User, Users, X } from "lucide-react"
...
const validateTaskForm = (formData, backendSchema) => {
  ...
  if (!formData.assigningToYourself) {
    if (!formData.taskAssignedTo || (Array.isArray(formData.taskAssignedTo) && formData.taskAssignedTo.length === 0)) {
...
const AssignTaskModal = ({ isOpen, onClose, task = null }) => {
  ...
  const [taskAssignedTo, setTaskAssignedTo] = useState([]) // Changed to array for multi-select
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (task) {
        ...
        if (selfFlag) {
          setTaskAssignedTo([]) // Reset to empty array
        } else {
          // Handle both single user and array of users
          if (Array.isArray(task.taskAssignedTo)) {
            setTaskAssignedTo(task.taskAssignedTo.map((u) => (typeof u === "string" ? u : u._id)))
          } else if (task.taskAssignedTo) {
            setTaskAssignedTo([typeof task.taskAssignedTo === "string" ? task.taskAssignedTo : task.taskAssignedTo._id])
          } else {
            setTaskAssignedTo([])
          }
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (assigningToYourself) {
        ...
        // Send taskAssignedTo as a single JSON array string
        if (Array.isArray(payload.taskAssignedTo)) {
          formData.append("taskAssignedTo", JSON.stringify(payload.taskAssignedTo))
        } else {
  ...
  const handleRemoveUser = (userId) => {
    setTaskAssignedTo((prev) => prev.filter((id) => id !== userId))
  }
  ...
  const handleDescriptionKeyDown = (e) => {
    if (e.key === "Enter" && !e.shiftKey) {
      ...
      if (numberMatch) {
        ...
        const newText = textBeforeCursor === "" ? `1. ${textAfterCursor}` : `${textBeforeCursor}\n1. ${textAfterCursor}`
        setTaskDescription(newText)

        // Set cursor position after "1. "
        setTimeout(() => {
          const newCursorPos = textBeforeCursor === "" ? 3 : cursorPosition + "\n1. ".length
          textarea.selectionStart = textarea.selectionEnd = newCursorPos
        }, 0)
      }
      ...
      setTimeout(() => {
        ...
        const hasNumbering = allLines.some((line) => /^\d+\.\s/.test(line.trim()))
  ...
  const selectedUserName = useMemo(() => {
    ...
    if (!taskAssignedTo || taskAssignedTo.length === 0) return "Select User(s)"
    
    // Show count of selected users
    const selectedCount = taskAssignedTo.length
    if (selectedCount === 1) {
      const user = allUsers.find((u) => u.id === taskAssignedTo[0])
      return user ? (
        <span className="flex items-center gap-1">
          <span className="truncate">{user.name}</span>
          <span className="bg-purple-200 rounded-md px-2 text-purple-600 font-medium text-xs">
            {user.department}
          </span>
        </span>
      ) : (
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              className={`h-10 break-words ${errors.taskTitle ? "border-red-500" : "border-gray-300"}`}
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              className={`min-h-[100px] break-words ${errors.taskDescription ? "border-red-500" : "border-gray-300"}`}
              style={{ overflowWrap: "break-word", wordBreak: "break-word" }}
              rows={4}
            />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                {!assigningToYourself && taskAssignedTo.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-2">
                    {taskAssignedTo.map((userId) => {
                      const user = allUsers.find((u) => u.id === userId)
                      if (!user) return null
                      return (
                        <div
                          key={userId}
                          className="flex items-center gap-1 bg-emerald-50 border border-emerald-200 rounded-full px-2 py-1 text-xs"
                        >
                          <img
                            ...
                            onError={(e) =>
                              (e.target.src = `/placeholder.svg?height=16&width=16&query=${encodeURIComponent(
                                user.name,
                              )}`)
                            }
                          />
                          <span className="text-emerald-700 font-medium truncate max-w-[100px]">{user.name}</span>
                          <button
                            type="button"
                            onClick={(e) => {
                              e.stopPropagation()
                  ...
                  <div className="absolute z-20 mt-1 w-full bg-white shadow-lg rounded-md border border-gray-300 flex flex-col max-h-72">
                    ...
                    <ul role="listbox" className="py-1 overflow-y-auto flex-grow">
                      {isFetchingUsers ? (
                        ...
                        filteredUsers.map((user) => {
                          const isSelected = taskAssignedTo.includes(user.id)
                          return (
                            <li
                              key={user.id}
                              role="option"
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <span className="block truncate font-medium">
                                  {user.name}{" "}
                                  <span className="bg-purple-200 rounded-md w-[fit-content] px-2 text-purple-600 font-medium text-xs ml-2 inline-block">
                                    {user.department}
                                  </span>
                                </span>
                    ...
                    {taskAssignedTo.length > 0 && (
                      <div className="border-t border-gray-200 p-2 flex-shrink-0">
                        ...
                        >
                          Done ({taskAssignedTo.length} selected)
                        </Button>
                      </div>
                    )}
                  </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-3 pt-2 justify-center sm:justify-start">
              ...
              <Button
                type="button"
                variant="outline"
                className="rounded-full p-2 h-10 w-10 border-gray-300 hover:bg-gray-50 bg-transparent"
                size="icon"
                title="Attach image"
              ...
              <p>Supported formats: Only Images (Max 1MB)</p>
            </div>
