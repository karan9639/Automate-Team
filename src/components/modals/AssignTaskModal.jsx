...
const AssignTaskModal = ({ isOpen, onClose, task = null }) => {
  ...
  const selectedUserName = useMemo(() => {
    ...
    return user ? (
      <span>
        {user.name} <span className="text-purple-400 font-medium text-xs ml-2">({user.department})</span>
      </span>
    ) : (
      "Select User"
    )
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <span className="block truncate font-medium">
                                {user.name}{" "}
                                <span className="text-purple-400 font-medium text-xs ml-2">({user.department})</span>
                              </span>
