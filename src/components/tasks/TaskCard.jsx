...
import { MoreHorizontal, Clock, CheckCircle, AlertCircle, Trash2 } from "lucide-react"
import { formatDate } from "../../utils/helpers"
import { changeTaskStatus, deleteTask as deleteTaskApi } from "../../api/tasksApi"
...
const TaskCard = ({ task, onClick, onStatusChange, onDelete, activeTab }) => {
  ...
  const extractTaskId = (taskToExtract) => {
    const possibleIds = [taskToExtract._id, taskToExtract.id, taskToExtract.taskId, taskToExtract.task_id]
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-3">
        <h3 className="font-semibold text-base sm:text-lg text-card-foreground">{taskTitle}</h3>
        <DropdownMenu modal={false}>
          <DropdownMenuTrigger asChild disabled={isUpdatingStatus || isRequestingDelete}>
            <button
          ...
          >
            <DropdownMenuItem
              ...
              disabled={isUpdatingStatus || isRequestingDelete || taskStatus === "in progress"}
            >
              {isUpdatingStatus && taskStatus !== "in progress" ? "Updating..." : "Mark as In Progress"}
            </DropdownMenuItem>
            <DropdownMenuItem
              onSelect={(e) => {
                e.preventDefault()
              ...
              disabled={isUpdatingStatus || isRequestingDelete || taskStatus === "completed"}
            >
              {isUpdatingStatus && taskStatus !== "completed" ? "Updating..." : "Mark as Completed"}
            </DropdownMenuItem>
            <DropdownMenuItem
              onSelect={(e) => {
                e.preventDefault()
              ...
              disabled={isUpdatingStatus || isRequestingDelete || taskStatus === "pending"}
            >
              {isUpdatingStatus && taskStatus !== "pending" ? "Updating..." : "Mark as Pending"}
            </DropdownMenuItem>
            {(activeTab === "delegated-tasks" || activeTab === "self-tasks") && (
              <>
                <DropdownMenuSeparator />
                <DropdownMenuItem
                  onSelect={(e) => {
      ...
      {taskDescription && <p className="text-muted-foreground text-sm mb-3">{taskDescription}</p>}
      <div className="flex flex-wrap gap-2 mb-3">
        <Badge variant="outline" className={`${getPriorityColor(taskPriority)} border`}>
          {taskPriority.charAt(0).toUpperCase() + taskPriority.slice(1)} Priority
        </Badge>
        <Badge
          variant="outline"
          className={`${getStatusColor(taskStatus)} ${isUpdatingStatus ? "opacity-75" : ""} border`}
        >
        ...
        {taskCategory && (
          <Badge variant="outline" className="bg-accent text-accent-foreground border">
            {taskCategory}
          </Badge>
        )}
      </div>
