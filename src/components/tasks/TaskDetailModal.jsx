...
import { Calendar, User, Flag, Clock, Edit, Trash2, CheckCircle, AlertCircle, Repeat } from "lucide-react"
import { format, formatDistanceToNow } from "date-fns"
import { fetchTaskById, changeTaskStatus, selectCurrentTask, selectTaskLoading } from "../../store/slices/taskSlice"
...
const TaskDetailModal = ({ isOpen, onClose, taskId, onEdit, onDelete }) => {
  ...
  const isOverdue = () => {
    if (!task?.taskDueDate || task.taskStatus?.toLowerCase() === "completed") return false
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-start">
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-2">
              <Badge variant="outline" className={`${getPriorityColor(task.taskPriority)} flex items-center`}>
                <Flag className="h-3 w-3 mr-1" />
                {task.taskPriority} Priority
              </Badge>
              <Badge
                variant="outline"
                className={`${getStatusColor(isOverdue() ? "overdue" : task.taskStatus)} flex items-center`}
              >
                {isOverdue() ? <AlertCircle className="h-3 w-3 mr-1" /> : <CheckCircle className="h-3 w-3 mr-1" />}
        ...
        {task.taskDescription && (
          <div>
            ...
            <p className="text-gray-700 bg-gray-50 p-4 rounded-md">{task.taskDescription}</p>
          </div>
        )}
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className={`text-gray-900 ${isOverdue() ? "text-red-600 font-medium" : ""}`}>
                  {task.taskDueDate ? format(new Date(task.taskDueDate), "PPP") : "No due date set"}
                </p>
                {task.taskDueDate && (
                  <p className="text-sm text-gray-500">
                    {formatDistanceToNow(new Date(task.taskDueDate), { addSuffix: true })}
                  </p>
                )}
              </div>
            </div>
            ...
            {task.taskFrequency?.type && (
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-gray-900 capitalize">{task.taskFrequency.type}</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="border-t pt-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              variant={task.taskStatus === "In Progress" ? "default" : "outline"}
              size="sm"
