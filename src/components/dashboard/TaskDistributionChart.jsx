...
const TaskDistributionChart = ({ tasks = [] }) => {
  ...
  useEffect(() => {
    const fetchTaskStats = async () => {
      try {
        ...
        console.log(
          "📊 Task Distribution Chart - API response:",
          response.data
        );
  ...
  // Special handling for single task - ensure it gets full circle
  if (segments.length === 1 && totalTasks === 1) {
    segments[0].angle = 360;
  }
  ...
  const createPieSegment = (
    ...
    // If this is a full circle (360 degrees), create two semicircles
    if (endAngle - startAngle >= 360) {
      ...
      return `M ${x1} ${y1} A ${radius} ${radius} 0 0 1 ${x2} ${y2} A ${radius} ${radius} 0 0 1 ${x1} ${y1} L ${x4} ${y4} A ${innerRadius} ${innerRadius} 0 0 0 ${x3} ${y3} A ${innerRadius} ${innerRadius} 0 0 0 ${x4} ${y4} Z`;
    }
